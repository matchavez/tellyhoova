...
## Layout
`Tellyhoova/` (ContentView.swift, TellyhoovApp.swift, `Models/`, `Services/`, `Views/`), `Tellyhoova.xcodeproj` (generated via XcodeGen from `project.yml` — don't hand-edit the xcodeproj if `project.yml` changes, regenerate with `xcodegen generate`), `Tellyhoova-1.0.{0,1,2,3,4,5}.dmg` — **note: these DMGs are committed directly to the repo root in addition to being attached as GitHub Release assets** (verified both exist for all 6 versions) — that's redundant repo bloat but appears intentional/unchanged across releases, not something to "clean up" without asking.
